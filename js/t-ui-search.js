...
var SearchWindow = {
    createNew: function() {
        ...
        //=====私有成员变量=====
        /** 界面html对象 */
        var $m_currentLayerPanel = $('.map-search-layer-current');
        ...
        //改变当前图层提示
        function setCurrentLayerInfo(layerobject) {
            if (layerobject) {
                g_oServerApi.setCurrentLayerObject(layerobject);
                var htmlinfo = "当前图层：" + layerobject.name;
                if ($m_currentLayerPanel.text() != htmlinfo) {
                    $m_currentLayerPanel.text(htmlinfo);
        ...
        function changeLayerMap(layerid) {
            var layerobject = g_oServerApi.getLayerObjectById(layerid);
